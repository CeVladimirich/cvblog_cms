...
echo '<tr><td align="center" colspan="2">Описание:<br><textarea rows="20" cols="100" name="desc">'.$desc.'</textarea></td></tr>';
echo '<tr><td align="center" colspan="2"><input type="submit" value="Добавить"></form></td></tr></table></center>';
break;
default:
echo '<a href="?page=post&topicid='.$tid.'&mode=addform"><b>ДОБАВИТЬ ПОСТ</b></a><br>';
$query = mysqli_query($dblink, "SELECT * FROM posts WHERE topicid = $tid AND postflag = 2 ORDER BY id DESC");
echo '<b>черновики</b><br>';
...
echo '<font color="grey"><b>подлежащие удалению</b><br>';
while($data = mysqli_fetch_array($query)) {
echo '<article class="post">';
echo '<div class="post-content">';
echo '<h2 class="post-title">'.$data['title'].'</h2>';
